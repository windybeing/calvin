...
int DeterministicLockManager::Lock(TxnProto* txn) {
  ...
  if (not_acquired > 0) {
    ...
    txn->set_lock_end_timestamp(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
  } else {
    ...
    if (it != requests->end()) {
      ...
      for (uint64 j = 0; j < new_owners.size(); j++) {
        ...
        if (txn_waits_[new_owners[j]] == 0) {
          ...
          // new_owners[j]->set_lock_end_timestamp(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
